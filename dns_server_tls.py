...
import translating_server_tls as translating_server
            ...
            while True:
                ...
                if dns_request.q.qtype == QTYPE.A or dns_request.q.qtype == QTYPE.AAAA:
                    ...
                    if target_domain not in self.domains_to_ports:
                        ...
                        self.domains_to_certs[target_domain] = (f'./certs/{target_domain}_chain.pem', f'./certs/{target_domain}.key')
                    ...
                    if self.current_translator is not None:
                        self.current_translator.terminate()
                    self.current_translator = translating_server.TranslatingServer(443,
                                                                                   self.domains_to_ports[target_domain],
                                                                                   self.domains_to_certs[target_domain],
                                                                                   self.quiet, self.verbose)
                    self.current_translator.start()
